 ...
 ;       setup-schedule
  ...
  ;    [
   ...
   ;   ]
    ...
    if test = true and ticks >= runtime
    [
...
to arrive [pack]
  ...
  [
      ...
      ifelse retardo > (tolerated-delay / 1000)
      [
...
to-report calculate-satisfaction [pack]
  ...
  ask pack
  [
    ifelse delay > (tolerated-delay / 1000)
    [
      set sigma 0
    ]
    [
      set sigma 1 - (delay / (tolerated-delay / 1000))
    ]
  ]
  let phi 0
...
to setup-nodes
  ...
  ask n-of number-of-sources (nodes with [shape = "circle" and distancexy 0 0 > 4 ])
  [
    setup-sources
  ]
end
...
to setup-topologia
  ...
  [
    ...
    [
      ...
      let tortuga-vecina one-of (other nodes in-radius coverage with [not link-neighbor? myself and count link-neighbors < average-degree-of-connectivity])
...
"routing" 1.0 0 -13791810 true "" "plot routing-losses-per-tick"
...
<experiments>
  <experiment name="sweeping sources and buffer" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>packets-sent-total</metric>
    ...
    <steppedValueSet variable="number-of-sources" first="1" step="1" last="30"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
