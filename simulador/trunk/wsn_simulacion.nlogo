...
to arrive [pack]
  ...
  [
      ...
      ifelse ticks - [creation-time] of pack > tolerated-delay
      [
  ...
  [
    ...
    ifelse idle and congestion-control
    [
      set idle false
      schedule-sent pack ticks + sent-time
    ]
...
to consume-energy
  ...
  [
    ...
    set total-routing-losses total-routing-losses + length buffer
    set routing-losses-per-tick routing-losses-per-tick + length buffer
    empty-buffer
    set size 0.5
  ]
end
...
to schedule-arrival [paquete destino tiempo]
  ...
  schedule-event evento-arribo
end

to schedule-event [evento]
  set agenda lput evento agenda
end

to schedule-sent [paquete tiempo]
...
to schedule-update-charts
  ...
  schedule-event evento-actualizacion
end
...
to sent [pack]
  ...
  [
    ...
    if not (not is-node? [next-node] of pack and [ttl] of pack = 1)
    [
    ...
    [
      ...
      ifelse length buffer > 0
      [
        schedule-sent 0 ticks + sent-time
      ]
    ...
    [
      if is-node? [next-node] of pack
      [
        ;si es la primera vez que se envía el paquete registra el envio de paquete
...
"" ""
PENS
"due to congestion" 1.0 0 -5825686 true "" "plot congestion-losses-per-tick"
"due to routing" 1.0 0 -955883 true "" "plot routing-losses-per-tick"
...
"" ""
PENS
"received" 1.0 1 -13840069 true "" "plotxy ticks (received-packets-per-tick)"
