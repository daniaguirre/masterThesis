...
to arrive [pack]
  ...
  [
    ...
    [
      set idle false
      schedule-sent pack ticks + sent-time "envio"
    ]
...
to congestion-control [pack]
  let friction sigma-x - [sigma] of pack
  let decision ""
  if friction < -5
  [
    set decision "dismiss"
  ]
  if friction >= -5 and sigma-x < -3
  [
    set decision "stop"
  ]
  set num-packet-fd num-packet-fd + 1
end
...
to-report generate-packet
  let arrival-time time-of-packet-generation
  let source self
  let pack nobody
  hatch-packets 1
  [
    set hidden? true
    set size .8
    set creation-time arrival-time
...
to place-packet-into-buffer [paquete]
  ifelse length buffer <= max-buffer-length or buffer-length = "infinito"
  [
...
to self-sent [pack]
  ...
  ifelse state != "muerto" and is-node? next-hub
  [
    ;agenda arribo en nodo destino
    ask pack
    [
    ...
    ;agenda un envio de paquete
    if active-sources
    [
      let new-pack generate-packet
      let time [creation-time] of new-pack + sent-time
      schedule-sent new-pack time "self-sent"
    ]
  ]
  [
    ask pack
...
to sent [pack]
  ...
  [
    ...
    ifelse length buffer > 0
    [
      schedule-sent 0 ticks + sent-time "envio"
    ]
...
to setup-schedule
  ...
  [
    ...
    let time [creation-time] of pack + sent-time
    schedule-sent pack time "self-sent"
  ]
...
to-report time-of-packet-generation
  ...
  [
    set tiempo-de-envio ticks + random-exponential (1 / arraival-rate)
  ]
  if packet-generator = "pdf-normal"
  [
    set tiempo-de-envio ticks + random-normal (1 / arraival-rate) 1
  ]
  if packet-generator = "continuous"
  [
    set tiempo-de-envio ticks + ( 1 / arraival-rate )
  ]
...
"" ""
PENS
"on time" 1.0 0 -16777216 true "" "plotxy ticks goodput"
"out time" 1.0 0 -9276814 true "" "plotxy ticks goodput-losses-per-tick"
...
"" ""
PENS
"due to congestion" 1.0 0 -11053225 true "" "plot congestion-losses-per-tick"
"due to wireless effects" 1.0 0 -16777216 true "" "plot wireless-losses-per-tick"
"due to routing" 1.0 0 -7500403 true "" "plot routing-losses-per-tick"
...
"" ""
PENS
"received" 1.0 0 -16777216 true "" "plotxy ticks received-packets-per-tick"
"sent" 1.0 0 -9276814 true "" "plotxy ticks sent-packets-per-tick"
